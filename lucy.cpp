...
namespace lucy
{
	...
	constexpr int countOf(const T (&)[count])
	{
		return count;
	};
...
#pragma optimize("", on)
	...
	struct WorkerTask
	{
		WorkerTask(System &system, u8 worker_index)
			: m_system(system), m_worker_index(worker_index)
		{
	...
	static void runInternal(void *data, void (*task)(void *), SignalHandle precondition, bool lock, SignalHandle *on_finish, u8 worker_index)
	{
		Job j;
		j.data = data;
		j.task = task;
